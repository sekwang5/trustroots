...
export default function OfferHostAvailability({ availability, onUpdateAvailability, maxGuests, onUpdateMaxGuests }) {
  return (
    <div className="row">
      <div className="col-xs-12 col-sm-6">
        ...
        <div className="panel panel-default offer-meta">
          <div className="panel-body">
            ...
            <AvailabilityInput value={availability} onChange={onUpdateAvailability} />

            <br /><br />

            {availability !== 'no' && <MaxGuestsInput value={maxGuests} onChange={onUpdateMaxGuests} />}
...
function AvailabilityInput({ value, onChange }) {
  ...
  return (
    <fieldset>
      <legend><h4 id="offerStatus">{t('Can you host?')}</h4></legend>
      <div className="btn-group"
        ...
        {availabilityOptions.map(({ label, status }) =>
          <label
            key={status}
            className={`btn btn-lg btn-offer-hosting btn-offer-hosting-${status} ${value === status ? 'active' : ''}`}
            role="radio"
